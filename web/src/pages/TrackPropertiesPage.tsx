  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="mt-3 flex shrink-0 flex-wrap items-center gap-2 border-t border-border pt-3" aria-label="Operation actions">
        <button type="button" onClick={() => { setIsSummaryExpanded(true); runPreview(); }} disabled={isApplying || preview.isPending || (selectedMkvPaths.length === 0 || !template)} className="h-9 min-w-0 flex-1 rounded-md border border-border bg-button px-3 text-sm font-semibold disabled:text-disabled">Preview Summary</button>
        {isApplying ? <button type="button" onClick={cancelRunningApply} disabled={cancelApply.isPending} className="h-9 min-w-0 flex-1 rounded-md border border-warning bg-button px-3 text-sm text-warning">Cancel</button> : <button type="button" onClick={runApply} disabled={apply.isPending || preview.isPending || (selectedMkvPaths.length === 0 || !template)} className="h-9 min-w-0 flex-1 rounded-md bg-accent px-3 text-sm font-semibold disabled:bg-button disabled:text-disabled">Apply</button>}
        <span className="w-full text-xs text-muted">{selectedMkvPaths.length} files selected</span>
