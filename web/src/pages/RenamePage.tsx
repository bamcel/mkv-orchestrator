  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="mt-3 flex shrink-0 flex-wrap items-center gap-2 border-t border-border pt-3" aria-label="Operation actions">
        <button type="button" onClick={() => { setIsSummaryExpanded(true); if (renameMode === "batch-movies") void previewBatchMovies(); else runPreview(); }} disabled={preview.isPending || apply.isPending || batchBusy || batchApplying || (renameMode === "batch-movies" ? batchMatches.every((item) => !item.results[item.selectedIndex]) : !selectedResult || selectedFiles.length === 0)} className="h-9 min-w-0 flex-1 rounded-md border border-border bg-button px-3 text-sm font-semibold disabled:text-disabled">Preview Summary</button>
        <button type="button" onClick={() => { if (renameMode === "batch-movies") void applyBatchMovies(); else void runApply(); }} disabled={preview.isPending || apply.isPending || batchBusy || batchApplying || (renameMode === "batch-movies" ? batchMatches.every((item) => !item.results[item.selectedIndex]) : !selectedResult || selectedFiles.length === 0)} className="h-9 min-w-0 flex-1 rounded-md bg-accent px-3 text-sm font-semibold disabled:bg-button disabled:text-disabled">Apply</button>
