...
export type MuxWorkflow = "remove" | "subtitles" | "convert";

export function MuxRemuxPage({ workflow = "remove" }: { workflow?: MuxWorkflow }) {
  ...
  const pageTitle = workflow === "remove" ? "Remove Tracks" : workflow === "subtitles" ? "Subtitles" : "Convert / Remux";
  const pageDescription = workflow === "remove"
    ? "Remove unwanted audio, subtitle, or specific tracks while preserving the container content you need."
    : workflow === "subtitles"
      ? "Mux external subtitle files or extract embedded subtitles."
      : "Convert MP4 containers to MKV without re-encoding the video or audio.";

  useEffect(() => {
    if (workflow === "convert" && mp4Files.length > 0) setConvertMp4(true);
  }, [workflow, mp4Files.length]);
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="grid min-h-0 min-w-0 flex-1 grid-cols-[minmax(14rem,18.75rem)_minmax(0,1fr)] gap-3">
        <section className="operation-sidebar flex min-h-0 flex-col overflow-hidden rounded-lg border border-border bg-card p-3 shadow-[0_1.25rem_3.75rem_rgba(0,0,0,0.18)]">
          ...
          {workflow === "convert" ? (
            <div className="mt-4 space-y-3 rounded-md border border-accent bg-panel p-3">
      ...
      <div className="mt-3 flex shrink-0 flex-wrap items-center gap-2 border-t border-border pt-3" aria-label="Operation actions">
        <button type="button" onClick={() => { setIsSummaryExpanded(true); runPreview(); }} disabled={isApplying || preview.isPending || (selectedMkvPaths.length === 0 && !(convertMp4 && selectedMp4Paths.length > 0))} className="h-9 min-w-0 flex-1 rounded-md border border-border bg-button px-3 text-sm font-semibold disabled:text-disabled">Preview Summary</button>
        {isApplying ? <button type="button" onClick={cancelRunningApply} disabled={cancelApply.isPending} className="h-9 min-w-0 flex-1 rounded-md border border-warning bg-button px-3 text-sm text-warning">Cancel</button> : <button type="button" onClick={runApply} disabled={apply.isPending || preview.isPending || (selectedMkvPaths.length === 0 && !(convertMp4 && selectedMp4Paths.length > 0))} className="h-9 min-w-0 flex-1 rounded-md bg-accent px-3 text-sm font-semibold disabled:bg-button disabled:text-disabled">Apply</button>}
        <span className="w-full text-xs text-muted">{selectedMkvPaths.length + (convertMp4 ? selectedMp4Paths.length : 0)} files selected</span>
